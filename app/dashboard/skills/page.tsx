...
export default function SkillPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setSkills(data.map(mapSkillFromApi));
    };
  ...
  return (
    <div className="p-8 mt-10 w-full max-w-1xl max-h-[85vh] overflow-y-auto no-scrollbar">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-white text-xl font-semibold">Skill</h1>
        ...
        <div className="flex gap-5 w-max">
          {skills.map((item) => (
            ...
            >
              ...
              <p className="mt-2 text-white">{item.image_title}</p>

              <div className="flex justify-center gap-2 mt-3">
                <button
                  onClick={() => router.push(`/dashboard/skills/${item.id}`)}
